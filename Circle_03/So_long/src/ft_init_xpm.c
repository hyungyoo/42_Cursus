...
{
	all->player.img = mlx_xpm_file_to_image(all->mlx.mlx, "./img/player.xpm", img_width, img_height);
	if (all->player.img == NULL)
		ft_print_error("Error: Not player image");
	all->wall.img = mlx_xpm_file_to_image(all->mlx.mlx, "./img/wall.xpm", img_width, img_height);
	if (all->wall.img == NULL)
		ft_print_error("Error: Not wall image");
	all->exit.img = mlx_xpm_file_to_image(all->mlx.mlx, "./img/exit.xpm", img_width, img_height);
	if (all->exit.img == NULL)
		ft_print_error("Error: Not exit image");
	all->floor.img = mlx_xpm_file_to_image(all->mlx.mlx, "./img/floor.xpm", img_width, img_height);
	if (all->floor.img == NULL)
		ft_print_error("Error: Not floor image");
	all->collectible.img = mlx_xpm_file_to_image(all->mlx.mlx, "./img/collectible.xpm", img_width, img_height);
...
void	ft_change_addr_player(t_info *all, int img_width, int img_height)
{
	...
	while (i < img_width)
	{
		...
		while (j < img_height)
		{
			if (all->player.addr[i * img_width + j] == c)
				all->player.addr[i * img_width + j] = all->floor.addr[i * img_width + j];
			j++;
		}
		i++;
	}
...
void	ft_change_addr_collectible(t_info *all, int img_width, int img_height)
{
	...
	while (i < img_width)
	{
		...
		while (j < img_height)
		{
			if (all->collectible.addr[i * img_width + j] == c)
				all->collectible.addr[i * img_width + j] = all->floor.addr[i * img_width + j];
			j++;
		}
		i++;
	}
