...
{
	ray->camera_x = 2 * pixel / (double)all->window_width - 1;
...
void	execute_dda(t_info *all, t_ray *ray)
{
	while (ray->hit == 0)
	{
		...
		if (all->map[ray->map_y][ray->map_x] == WALL)
			ray->hit = 1;
	}
}
...
{
	ray->line_height = (int)(all->window_height / ray->perp_wall_dist);
	ray->draw_start = -ray->line_height / 2 + all->window_height / 2;
	if (ray->draw_start < 0)
		ray->draw_start = 0;
	ray->draw_end = ray->line_height / 2 + all->window_height / 2;
	if (ray->draw_end >= all->window_height)
		ray->draw_end = all->window_height - 1;
}
